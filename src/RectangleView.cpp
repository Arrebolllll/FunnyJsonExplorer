...
void
RectangleView::show(Component* root) {
...
void
RectangleView::getRecInfo(Component* it, int depth) {
    jsonSize++;
...
int
RectangleView::getLineLen(Component* it, int depth) {
    int len;
...
void
RectangleView::Visualize(Component* it, int depth) {
    int len = getLineLen(it, depth);
    outputCount++;
