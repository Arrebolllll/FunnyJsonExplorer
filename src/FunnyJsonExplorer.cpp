...
bool
FunnyJsonExplorer::load(const std::string& file) {
    try {
...
Component*
FunnyJsonExplorer::json_to_component(const json& j,
...
void
FunnyJsonExplorer::setIcon(const std::string& _middle_icon,
...
void
FunnyJsonExplorer::setView(std::unique_ptr<View>&& v) {
    _view = std::move(v);
}

void
FunnyJsonExplorer::show() {
    if (_view && root) {
        _view->show(root.get());
    }
}